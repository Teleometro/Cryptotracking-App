...
public class GameBoard {
    ...
    private static final String dbName = "saves.db";
    private static final String dbURL = "jdbc:sqlite:saves.db";
    ...
    public void initDB() {
        ...
        String createSavesTableSQL =
                """
                CREATE TABLE IF NOT EXISTS saves (
                    username text NOT NULL,
                    savename text NOT NULL,
                    data text NOT NULL,
                    PRIMARY KEY (username, savename)
                );
                """;
        try (Connection conn = DriverManager.getConnection(dbURL);
            Statement statement = conn.createStatement()) {
            statement.execute(createSavesTableSQL);
    ...
    public void save(String username, String savename) {
        String addSingleStudentWithParametersSQL =
                """
                INSERT INTO currency(name, symbol, id) VALUES
                    (?, ?, ?)
                """;
        try (Connection conn = DriverManager.getConnection(dbURL);
            ...
            preparedStatement.setString(3, serialise());
    ...
    public void updateSave(String username, String savename) {
        String addSingleStudentWithParametersSQL =
                """
                    UPDATE saves
                    SET data = ?
                    WHERE username = ? AND savename=?
                """;
        try (Connection conn = DriverManager.getConnection(dbURL);
             PreparedStatement preparedStatement = conn.prepareStatement(addSingleStudentWithParametersSQL)
            ) {
            ...
            preparedStatement.setString(1, serialise());
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            System.out.println(e.getMessage());
            System.exit(-1);
    ...
    public Boolean checkForSave(String username, String savename) {
        String addSingleStudentWithParametersSQL =
                """
                SELECT COUNT(*) FROM saves WHERE username=? AND savename=?
                """;
        try (Connection conn = DriverManager.getConnection(dbURL);
             PreparedStatement preparedStatement = conn.prepareStatement(addSingleStudentWithParametersSQL)
            ) {
    ...
    public Map<String, String> listSaves(String username) {
        Map<String, String> saves = new HashMap<>();
        String addSingleStudentWithParametersSQL =
                """
                SELECT savename, data FROM saves WHERE username=?
                """;
        try (Connection conn = DriverManager.getConnection(dbURL);
             PreparedStatement preparedStatement = conn.prepareStatement(addSingleStudentWithParametersSQL)
            ) {
            ...
            while (result.next()) {
                saves.put(result.getString("savename"), result.getString("data"));
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
            System.exit(-1);
